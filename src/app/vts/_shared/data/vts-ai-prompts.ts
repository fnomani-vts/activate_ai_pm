...
export const vtsAiPromptsWithoutContext: VtsAiPrompt[] = [
  ...
  },
  {
    ...
    payload: {
      ...
      summary: [
        "The 5 largest vacant spaces in Canada with no active deals total 817,885 square feet. The largest is a 326,105 sq ft space in '1243 Fourth Line' (Space 0001A).",
      ],
      keyInsights: [
        "The top 5 spaces are:",
        "1. '1243 Fourth Line' (Space 0001A) at 326,105 sq ft",
        "2. '2324 - 123 Street' (Space 1234) at 236,433 sq ft",
        "3. 'Tacklewinds - Bldg B' (Space 5432) at 123,360 sq ft",
        "4. 'St Martin Centre' (Space 0003) at 234,023 sq ft",
        "5. 'Laurier Square' (Space 0004) at 112,205 sq ft",
        "",
        "These 5 spaces account for 23.5% of the total 3,477,770 square feet of vacant space listed.",
        "The largest space, 326,105 sq ft in '1243 Fourth Line', represents 12.2% of the total vacant space by itself.",
      ],
      suggestedFollowUps: [
        "How are concessions trending year over year?",
        "Which submarkets have the highest unique tenant tours this month?",
  ...
  },
  {
    ...
    payload: {
      ...
      keyInsights: [
        ...
        "- 'Bob's Petfoods Inc.'",
        "- '1235675 Ontario Inc.'",
        "- 'Clairs International Inc.'",
        "- 'NextGen Group Ltd.'",
        "- 'Rob Consulting Inc.'",
        "- 'Toronto Parts & Service Ltd.'",
        "",
        "The dates for these 'negative' status updates range from September 2023 to September 2025.",
      ],
      suggestedFollowUps: [
  ...
  },
  {
    ...
    payload: {
      ...
      summary: [
        "Within the 18-month lease roll, 111 unique tenants were identified as Canadian, totaling 2,823,249 square feet.",
        "",
        "The largest are:",
        "- 'Best Buy Canada Inc.' (625K sq ft)",
        "- 'Novo Nordisk Canada Limited' (360K sq ft)",
        "- 'The Bank Of Toronto' (230K sq ft)",
      ],
      keyInsights: [
        "The top 3 tenants account for 1,012,487 square feet, representing 32.1% of the total expiring square footage for Canadian-identified tenants.",
        "",
        "The 5 largest Canadian tenants by expiring area are:",
        "1. 'Best Buy Canada Inc.' (625,135 sq ft)",
        "2. 'Novo Nordisk Canada Limited' (360,234 sq ft)",
        "3. 'The Bank Of Toronto' (230,669 sq ft)",
        "4. 'Clairs Canada Inc.' (138,423 sq ft)",
        "5. 'Vancouver Health Services' (102,571 sq ft)",
        "",
        "The smallest single expiring lease identified as Canadian is 0 square feet for 'Red River Vending Solutions-Canada, ULC'.",
      ],
      suggestedFollowUps: [
  ...
  },
  {
    ...
    payload: {
      ...
      summary: [
        "RBC Bank: Holds 4 leases across both countries (2 in the US, 4 in Canada), totaling 32,470 RSF and generating $2,546,079 in revenue.",
        "",
        "Sherman Williams & Co.: Has 2 leases (1 in each country) for a combined 213,151 RSF and $4,120,785 in revenue.",
      ],
      keyInsights: [
        "Sherman Williams & Co.:",
        "- Most valuable lease: '8350 Robert Street' (US), 123,844 sq ft generating $2.55M at $21.81/sf, expires March 31, 2027",
        "- Second lease: 'Saint Martins Industrial' (Canada), 73,312 sq ft generating $1.26M at $17.12/sf, expires May 28, 2030",
        "",
        "RBC Bank's portfolio:",
        "- US lease: '785 Fourth Avenue' (4,845 sq ft), generates $1.24M at $226.13/sf, expires September 30, 2031",
        "- Canadian retail-center leases:",
        "  • 'Yorkdale Shopping Centre' (expires February 28, 2027, at $74.10/sf)",
        "  • 'Laurier Square' (expires June 30, 2027, at $63.17/sf)",
        "  • 'Markham Shopping Centre' (expires February 29, 2028, at $49.22/sf)",
        "",
        "Critical expiration date: February 28, 2027 is a critical date, as both RBC Bank's 'Yorkdale Shopping Centre' lease (at $74.10/sf) and Sherman Williams & Co.'s high-revenue '8350 Robert Street' lease (at $20.82/sf) expire. This is followed just four months later by RBC Bank's 'Laurier Square' lease on June 30, 2027, grouping three of the six leases into a key renewal period.",
      ],
      suggestedFollowUps: [
        "What's the average rent right now?",
        "How are rents trending year-over-year?",
...
// Deep copy helper function to ensure complete data isolation
function deepCopyPrompts(prompts: VtsAiPrompt[]): VtsAiPrompt[] {
  return JSON.parse(JSON.stringify(prompts)) as VtsAiPrompt[];
}

// New isolated prompt sets for different routes
// Deals prompts (main page - without context)
export const vtsAiPromptsDeals: VtsAiPrompt[] = [...vtsAiPromptsWithoutContext];

// Deals prompts (profile page - with context)
export const vtsAiPromptsDealsWithContext: VtsAiPrompt[] = [...vtsAiPromptsWithContext];

// BGO prompts (main page - without context) - initially a copy of deals
// export const vtsAiPromptsBgo: VtsAiPrompt[] = deepCopyPrompts(vtsAiPromptsWithoutContext);
export const vtsAiPromptsBgo: VtsAiPrompt[] = [
  {
    prompt: "What's the average rent right now?",
    payload: {
      marketMetadata: {
...
// BGO prompts (profile page - with context) - initially a copy of deals
export const vtsAiPromptsBgoWithContext: VtsAiPrompt[] = deepCopyPrompts(vtsAiPromptsWithContext);



// Centralized prompt selection helper
export const getPromptsForPath = (pathname: string | null): VtsAiPrompt[] => {
  if (!pathname) return vtsAiPromptsDeals;
  
  if (pathname === "/vts/lease/deals/profile") {
    return vtsAiPromptsDealsWithContext;
  }
  if (pathname === "/vts/lease/deals") {
    return vtsAiPromptsDeals;
  }
  if (pathname === "/vts/lease/dealsbgo/profile") {
    return vtsAiPromptsBgoWithContext;
  }
  if (pathname === "/vts/lease/dealsbgo") {
    return vtsAiPromptsBgo;
  }
  
  // Fallback to deals prompts for unknown paths
  return vtsAiPromptsDeals;
};
